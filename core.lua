...
local function isBlacklistedAmber(amber)
    ...
    for _, cf in ipairs(blacklistCFrames) do
        if (pos - cf.Position).Magnitude <= POSITION_THRESHOLD then return true end
    end
    return false
end
...
pcall(function()
    ...
    if amberFolder then
        for _, amber in pairs(amberFolder:GetChildren()) do
            if isBlacklistedAmber(amber) then ignoredAmbers[amber] = true end
        end
    end
end)
...
local function scanForVisibleGuis()
    ...
    local function scanGui(obj)
        if obj:IsA("GuiObject") and obj.Visible then currentGuis[getFullPath(obj)] = true end
        for _, sub in ipairs(obj:GetChildren()) do scanGui(sub) end
    end
                        ...
                        if not ignoredAmbers[amber] and amber.Name == "AmberSpawn" then
                            ...
                            if prompt and prompt.Enabled then
                                ...
                                if pos then
                                    ...
                                    if not inside then table.insert(prompts, prompt) end
                                end
                            end
                        end
                    end
        ...
        hum.Died:Connect(function()
            ...
            task.spawn(function()
                ...
                while Config.AutoRespawn and attempts < 30 do
                    ...
                    if attempts > 2 then pcall(function() executeSpawn() end) end
                    attempts = attempts + 1
...
return {
    ...
    setAutoSpawn = function(val)
        AUTO_SPAWN_ENABLED = val
        if val then lastVisibleGuis = {} end
    end,
    setRespawnConnection = function(val) respawnConnection = val end,
    getRespawnConnection = function() return respawnConnection end,
}